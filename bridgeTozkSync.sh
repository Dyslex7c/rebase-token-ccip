...
ZKSYNC_REBASE_TOKEN_ADDRESS=$(forge create src/RebaseToken.sol:RebaseToken --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myaccount --legacy --zksync | awk '/Deployed to:/ {print $3}')
...
cast send $ZKSYNC_POOL_ADDRESS "applyChainUpdates((uint64,bool,bytes,bytes,(bool,uint128,uint128),(bool,uint128,uint128))[])" "[($SEPOLIA_CHAIN_SELECTOR,true,$(cast abi-encode "f(address)" $SEPOLIA_POOL_ADDRESS),$(cast abi-encode "f(address)" $SEPOLIA_REBASE_TOKEN_ADDRESS),(false,0,0),(false,0,0))]" --rpc-url $ZKSYNC_SEPOLIA_RPC_URL --account myaccount
...
forge script ./script/BridgeTokens.s.sol:BridgeTokensScript --rpc-url ${SEPOLIA_RPC_URL} --account myaccount --broadcast --sig "run(address,uint64,address,uint256,address,address)" $(cast wallet address --account myaccount) ${ZKSYNC_SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${AMOUNT} ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
